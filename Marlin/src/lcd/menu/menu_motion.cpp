...
#define _HOME_ITEM(N) GCODES_ITEM_N(N##_AXIS, MSG_AUTO_HOME_A, F("G28" STR_##N));
  ...
  void menu_ft_motion() {
    ...
    if (c.mode) {    
      ...
      #if HAS_X_AXIS
        if WITHIN(c.cmpnstr[X_AXIS], ftMotionCmpnstr_ZV, ftMotionCmpnstr_MZV)
      ...
      #endif
      #if HAS_Y_AXIS
        if WITHIN(c.cmpnstr[Y_AXIS], ftMotionCmpnstr_ZV, ftMotionCmpnstr_MZV)
      ...
      #if HAS_X_AXIS
        if WITHIN(c.cmpnstr[X_AXIS], ftMotionCmpnstr_EI, ftMotionCmpnstr_3HEI)
          EDIT_ITEM_FAST_N(float42_52, X_AXIS, MSG_FTM_VTOL_N, &c.vtol[0], 0.0f, 1.0f, ftMotion.update_shaping_params);
      #endif
      #if HAS_Y_AXIS
        if WITHIN(c.cmpnstr[Y_AXIS], ftMotionCmpnstr_EI, ftMotionCmpnstr_3HEI)
